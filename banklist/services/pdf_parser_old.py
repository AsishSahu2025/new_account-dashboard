        ...
        try:
            ...
            for top in sorted(rows):
                ...
                if is_txn:
                    ...
                    # Continuation row (wrapped narration, second/third line
                    # of a multi-line description). NOTE: if this continuation
                    # is the very first row on a NEW page (i.e. the previous
                    # page ended mid-narration), `pending` has already been
                    # reset to None for this page and the text is dropped —
                    # this only affects the description text, never amounts,
                    # since Withdrawal/Deposit/Balance/Ref/Date always sit on
                    # a transaction's first line, which is always fully
                    # contained within a single page in every sample checked.
                    if pending:
                        for w in rw:
                            txt = w['text']
                            if _in_hdfc(w, 'narration'):
