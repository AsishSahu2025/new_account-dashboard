...
# def register_strategy(strategy_name: str, parser) -> None:
#     """Register one global extraction strategy."""
#     if not strategy_name:
#         raise ValueError("Strategy name cannot be empty.")

#     if not callable(parser):
#         raise TypeError("Strategy parser must be callable.")

#     STRATEGY_REGISTRY[strategy_name] = parser


# def get_strategy(strategy_name: str):
#     """Return the parser registered for a global strategy."""
#     if not strategy_name:
#         raise ValueError("Strategy name cannot be empty.")

#     parser = STRATEGY_REGISTRY.get(strategy_name)
...
# # =========================================================
# # GLOBAL COORDINATE STRATEGY
# # =========================================================

# def _parse_coordinate(pdf, config=None, field_aliases=None) -> tuple:
#     """
#     Global coordinate-based extraction entry point.

#     The existing tested HDFC implementation is intentionally
#     kept unchanged during this migration step.

#     config and field_aliases are accepted now so the database
#     configuration can be connected later.
#     """
#     return _parse_hdfc(pdf)
...
#     field_aliases=None,
# ) -> tuple:
#     """Execute a registered global extraction strategy."""
#     parser = get_strategy(strategy_name)
...
def _get_bank_statement_configuration(bank_short_name: str):
    """Return the active database configuration for a bank."""
    try:
        from banklist.models import Bank, BankStatementConfig
    except Exception:
        return None
    ...
    bank = (
        Bank.objects
        .filter(
            short_name__iexact=bank_short_name,
            is_active=True,
        )
        .first()
    )
...
def _get_configured_field_aliases(config) -> dict:
    """Return active semantic field aliases from the database."""
    if config is None:
        return {}

    aliases = defaultdict(list)

    for item in config.field_aliases.filter(
        is_active=True
    ).order_by("id"):
        alias = str(item.alias or "").strip()

        if alias:
            aliases[item.field_name].append(alias)
...
def _validate_strategy_configuration(
    ...
    """Validate database configuration only."""
    if config is None:
        raise ValueError(
            "Bank statement configuration was not found."
        )

    if not config.is_active:
        raise ValueError(
            "Bank statement configuration is inactive."
        )

    if config.extraction_strategy not in {
        STRATEGY_COORDINATE,
        STRATEGY_TABLE,
        STRATEGY_OCR,
    }:
        raise ValueError(
            ...
            f"{config.extraction_strategy}"
        )

    if not field_aliases:
        raise ValueError(
            "No active field aliases configured for "
            f"{config.bank.bank_name}."
        )
...
def _get_database_strategy(
    ...
    """Return (config, field_aliases) for a configured bank."""
    config = _get_bank_statement_configuration(
        bank_short_name
    )
    ...
    field_aliases = _get_configured_field_aliases(
        config
    )
        ...
        for window in windows:
            ...
            for start_index in range(
                ...
                active_aliases = (
                    field_aliases
                    if field_aliases
                    else _HDFC_HEADER_ALIASES
                )

                for field, aliases in (
                    active_aliases.items()
                ):
...
def _parse_coordinate(
    ...
    """
    Global coordinate-based extraction strategy.

    Existing HDFC extraction and validation remain unchanged.
    Only the header aliases are supplied from configuration.
    """
    return _parse_hdfc(
        pdf,
        field_aliases=field_aliases,
...
def _parse_ocr_strategy(
    ...
    Uses OCR text plus the existing Ollama transaction
    ...
    if not full_text:
        return [], [
            "OCR strategy could not extract text."
        ]

    ai_result = _ollama_statement_result(
        full_text
    )

    if not ai_result:
        return [], [
            "OCR strategy extracted text but no "
    ...
    def parse_pdf(self, file_bytes: bytes, file_name: str = "") -> ParseResult:
        try:
            ...
            with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
                ...
                # Configured banks use the database-selected
                # global extraction strategy.
                #
                # Banks without a configuration keep the
                # existing tested routing as a safe fallback.

                config, field_aliases = (
                    _get_database_strategy(bank)
                )

                if config is not None:

                    txns, errors = _parse_using_strategy(
                        pdf=pdf,
                        strategy_name=config.extraction_strategy,
                        config=config,
                        field_aliases=field_aliases,
                    )
